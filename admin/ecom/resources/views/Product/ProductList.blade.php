            ...
            axios.get(URL).then(function (response) {
                if(response.status==200){
                    ...
                    $.each(response.data,function (i,item){
                        let tableRow="<tr>" +
                            "<td>"+i+"</td>" +
                            "<td><img style='height: 50px;width: 50px;' src='"+item['product_image']+"'></td>" +
                            ...
                            "<a class='dropdown-item deleteItem' data-id="+item['id']+" data-img="+item['product_image']+" href='#'>Delete</a>"+
                            "<a class='dropdown-item changeImage' data-id="+item['id']+" data-img="+item['product_image']+" href='#'>Change Image</a>"+
            ...
            axios.get(URL,AxiosConfig).then(function (response){
                if(response.status===200){
                    ...
                    $.each(JSONData, function (key, value) {
                        let OneData="<option value='"+value['category_name']+"'>"+value['category_name']+"</option>";
        ...
        $('#category').on('change',function (){
            ...
            axios.post(URL,{category_name:category_name},AxiosConfig).then(function (response){
                if(response.status===200){
                    ...
                    console.log(JSONData);
                    $.each(JSONData, function (key, value) {
                        let OneData="<option value='"+value['subcategory_name']+"'>"+value['subcategory_name']+"</option>";
        ...
        function GetProductListEditData(idDetails){
            ...
            axios.post(URL,{id:idDetails},AxiosConfig).then(function (response) {
                ...
                if (response.status===200) {
                    ...
                    $('#title_edit').val(EditData[0]['product_name']);
                    $('#price_edit').val(EditData[0]['price']);
                    $('#special_price_edit').val(EditData[0]['discount_price']);
                    ...
                    $('#star_edit').val(EditData[0]['rating']);
